...
# Secure Pi Setup using Pi-hole with DNSCrypt and Tailscape
```mathematica
Clients → Pi-hole → DNSCrypt → Internet
VPN clients → Tailscape → Wireguard → Pi-hole → DNSCrypt → Internet
```
- Pi-hole = DNS filtering / DHCP (optional)
- dnscrypt-proxy = encrypted upstream DNS
- Tailscape (WireGuard) = secure remote access - only one I got to work with LAN/WAN
...
ipv6_servers = false        # set false if not using IPv6
dnssec = true
require_dnssec = true
```
...
sudo pihole restartdns #if it fails use 'sudo pihole reloaddns'
```
